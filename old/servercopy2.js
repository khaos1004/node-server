...
try {
    mobileOK= require("../mok/mok_Key_Manager_v1.0.3.js"); 
} catch (error)  {
...
const requestUri = 'http://localhost:8085/mok/mok_std_request';  // mok 인증 요청 URI  
const resultUri = 'http://localhost:8085/mok/mok_std_result';  // mok 결과 요청 URI


/* 2-3 결과 수신 후 전달 URL 설정 - "https://" 포함한 URL 입력 */
/* 결과 전달 URL 내에 개인정보 포함을 절대 금지합니다.*/
// const resultUrl = 'https://이용기관URL/mok/mok_std_result'; 
const resultUrl = 'http://localhost:8085/mok/mok_std_result'; 

/* 3. 본인확인 서비스 API 설정 */
/* 3-1 키파일 경로(본인확인 키정보파일 Path)설정 */
/* 키파일은 반드시 서버의 안전한 로컬경로에 별도 저장. 웹URL 경로에 파일이 있을경우 키파일이 외부에 노출될 수 있음 주의 */
// const keyPath = './mok/fakekey/mok_keyInfo.dat';
const keyPath = './mok/fakekey/mok_keyInfo.dat';
